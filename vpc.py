...
# This script is part of a larger process that is initiated via a Jenkins job.  This script contributes the following to that process:
# 1.  Checks input parameters to make sure they're valid
# 2.  Finds an available IP block based on the requested IP allocation size
# 3.  Generates the necessary Terraform source in the account's source directory in 'shared_services_consumers' for creating the VPC
...
# base_region - check it matches allowed/supported regions, including environment checks
...
# ip_allocation - check it's a suitable size
...
# vpc_name - check it doesn't already exist
